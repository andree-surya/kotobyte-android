...
public class KanjiStrokesTextureView extends TextureView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mVectorPaths == null || mVectorPaths.size() == 0) {
            ...
            int numberOfColumns = (int) Math.floor(parentWidth / (mGridSize + mGridSpacing));
    ...
    private void redrawKanjiStrokes() {
        Canvas canvas = lockCanvas();

        float drawingScale = mGridSize / DOCUMENT_SIZE;
        int numberOfColumns = (int) Math.floor(canvas.getWidth() / (mGridSize + mGridSpacing));
